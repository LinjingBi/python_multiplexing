...
if __name__ == '__main__':
    gen = gen_func(12) #gen_func（12）并不会执行，因为生成器只有实列化后，通过对实例进行next操作才会开始执行，这一点不同于普通函数传入参数就会开始执行。
    print(next(gen)) #0. 12
    ...
    try:
        gen.throw(Exception,'download error') #throw会在上一步已经执行完的yield报错（yield 3），可以用try pass掉，主程序继续执行，但是生成器已经close，就算后面有yield，也会抛SI
